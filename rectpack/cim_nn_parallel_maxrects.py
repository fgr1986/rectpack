...
class CIMNNParallelMaxRects(PackingAlgorithm):

    def __init__(self, width, height, rot=True, *args, **kwargs):
        super(CIMNNParallelMaxRects, self).__init__(
            width, height, rot, *args, **kwargs)
        print('[debug] Custom CIMNNParallelMaxRects algorithm')
    ...
    def _select_position(self, w, h, rid, rect_list):
        """
        ...
        _r_l = rid.split('_')[0]
        _rect_list_l = [r[4].split('_')[0] for r in rect_list]

        # Normal rectangle
        fitn = ((self._rect_fitness(m, w, h), w, h, m)
                for m in self._max_rects
                if (self._rect_fitness(m, w, h) is not None and
                    # _r_l not in _rect_list_l
                    len(_rect_list_l) == 0
                    ))
    ...
    def fitness(self, width, height, rid, rect_list):
        """
        ...
        rect, max_rect = self._select_position(width, height, rid, rect_list)
        if rect is None:
            return None
    ...
    def add_rect(self, width, height, rid, rect_list):
        """
    ...
    def reset(self):
        super(CIMNNParallelMaxRects, self).reset()
        self._max_rects = [Rectangle(0, 0, self.width, self.height)]


class CIMNNParallelMaxRectsBl(CIMNNParallelMaxRects):
...
class CIMNNParallelMaxRectsBssf(CIMNNParallelMaxRects):
...
class CIMNNParallelMaxRectsBaf(CIMNNParallelMaxRects):
...
class CIMNNParallelMaxRectsBlsf(CIMNNParallelMaxRects):
